...
def main():
    ...
    # текст:
    ap.add_argument("--numbers_mode", choices=["keep","spell"], default="keep")
    ap.add_argument("--latin_mode", choices=["keep","translit"], default="keep")
    # post-train eval:
    ap.add_argument("--eval_after_training", action="store_true",
                    help="посчитать WER после обучения (best.pt по умолчанию)")
    ap.add_argument("--eval_split", choices=["validation","test"], default="test",
    ...
    print("Checkpoint 1")
    # loaders + refs
    dl_tr, dl_va, refs_va, vocab, normalizer = build_loaders(
    ...
    best_path, best_wer = trainer.fit(dl_tr, dl_va, refs_valid=refs_va, epochs=args.epochs)

    print("Checkpoint 4")

    # optional: evaluate WER after training
    results = {"val_best_wer": best_wer, "best_ckpt": best_path}
    if args.eval_after_training:
        ...
        results[f"{args.eval_split}_wer"] = float(eval_wer)
        results["eval_ckpt"] = ckpt_path
    ...
    print(f"[metrics] saved -> {os.path.join(args.out, 'metrics.json')}")
